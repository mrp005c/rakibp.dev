...
const Header = () => {
  ...
  return (
    ...
    >
      <div className="flex-between p-3 box-border max-w-[1280px] mx-auto h-full">
        <Logo />
        <nav className="flex-between hidden md:flex gap-3">
          <ul className=" text-black/80  flex-between gap-3">
            {Nav_Links.map((item, index) => (
              <li key={index} className="hover:scale-110 hover:text-black transition-all">
          ...
          <DrawerContent className="p-2 z-9999">
            ...
            <div className="no-scrollbar overflow-y-auto ">
              <ul className=" flex-col w-full flex text-black/80  gap-3 border border-gray/80 rounded-md py-2">
                {Nav_Links.map((item, index) => (
                  <li key={index} className="hover:bg-gray/40 rounded-sm box-border px-2 w-full" >
                    <Link
                      href={item.href}
